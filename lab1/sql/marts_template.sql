...
select
    ...
    coalesce(mcc.category, 'Неизвестно')        as category,-- TODO: категория трат из staging.mcc (join по mcc);
    --       если MCC в справочнике нет — 'Неизвестно' (coalesce)
    m.channel
from staging.merchants m
left join staging.mcc  mcc on lower(mcc.mcc) = lower(m.mcc) -- TODO: left join staging.mcc ...
...
select
    ...
    d.date_sk,
    t.txn_ts,
    coalesce(dc.client_sk, -1)                       as client_sk,-- TODO: client_sk — из dc, но если join не нашёл клиента,
    --       операция должна попасть на «неизвестного» (ключ -1, coalesce)
    coalesce(dp.product_sk, -1)                       as product_sk, -- TODO: product_sk — так же
